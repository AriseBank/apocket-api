 ...
 * @param {Number} options.apwsPort - Port for aPocket Wallet Service API
 * @param {Number} options.messageBrokerPort - Port for APWS message broker
 * @param {Number} options.lockerPort - Port for APWS locker port
 */
var Service = function(options) {
  ...
  this.apwsPort = options.apwsPort || baseConfig.port;
...
Service.prototype._startWalletService = function(config, next) {
  ...
  expressApp.start(config, function(err){
    ...
    self.server.listen(self.apwsPort, next);
  });
};
