...
/**
 * Creates an instance of the aPocket Wallet Service.
 * @constructor
 */
...
/**
 * Gets the current version of APWS
 */
WalletService.getServiceVersion = function() {
  if (!serviceVersion)
    serviceVersion = 'apws-' + require('../package').version;
...
/**
 * Gets an instance of the server after authenticating the pocketeer.
 * @param {Object} opts
 * @param {string} opts.pocketeerId - The pocketeer id making the request.
 * @param {string} opts.message - (Optional) The contents of the request to be signed. Only needed if no session token is provided.
 * @param {string} opts.signature - (Optional) Signature of message to be verified using one of the pocketeer's requestPubKeys. Only needed if no session token is provided.
 * @param {string} opts.session - (Optional) A valid session token previously obtained using the #login method
 * @param {string} opts.clientVersion - A string that identifies the client issuing the request
 * @param {string} [opts.walletId] - The wallet id to use as current wallet for this request (only when pocketeer is support staff).
 */
WalletService.getInstanceWithAuth = function(opts, cb) {
  function withSignature(cb) {
    if (!checkRequired(opts, ['pocketeerId', 'message', 'signature'], cb)) return;
    ...
    server.storage.fetchPocketeerLookup(opts.pocketeerId, function(err, pocketeer) {
      if (err) return cb(err);
      if (!pocketeer) return cb(new ClientError(Errors.codes.NOT_AUTHORIZED, 'Pocketeer not found'));

      if (!pocketeer.isSupportStaff) {
        var isValid = !!server._getSigningKey(opts.message, opts.signature, pocketeer.requestPubKeys);
        if (!isValid)
          return cb(new ClientError(Errors.codes.NOT_AUTHORIZED, 'Invalid signature'));
        server.walletId = pocketeer.walletId;
      } else {
        server.walletId = opts.walletId || pocketeer.walletId;
        server.pocketeerIsSupportStaff = true;
      }

      server.pocketeerId = opts.pocketeerId;
      return cb(null, server);
    });
  };

  function withSession(cb) {
    if (!checkRequired(opts, ['pocketeerId', 'session'], cb)) return;
    ...
    server.storage.getSession(opts.pocketeerId, function(err, s) {
      if (err) return cb(err);
...
WalletService.prototype.login = function(opts, cb) {
  ...
  async.series([
    ...
    function(next) {
      self.storage.getSession(self.pocketeerId, function(err, s) {
    ...
    if (err) return cb(err);
    if (!session) return cb(new Error('Could not get current session for this pocketeer'));
...
WalletService.prototype.logout = function(opts, cb) {
  ...
  self.storage.removeSession(self.pocketeerId, cb);
};
 ...
 * @param {number} opts.m - Required pocketeers.
 * @param {number} opts.n - Total pocketeers.
 * @param {string} opts.pubKey - Public key to verify pocketeers joining have access to the wallet secret.
...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  if (!Wallet.verifyPocketeerLimits(opts.m, opts.n))
    return cb(new ClientError('Invalid combination of required pocketeers / total pocketeers'));
 ...
 * @param {Object} opts.includeExtendedInfo - Include PKR info & address managers for wallet & pocketeers
  ...
  async.parallel([
    ...
    function(next) {
      self.getWallet({}, function(err, wallet) {
        ...
        var pocketeerExtendedKeys = ['xPubKey', 'requestPubKey', 'signature', 'addressManager', 'customData'];

        wallet.pocketeers = _.map(wallet.pocketeers, function(pocketeer) {
          if (pocketeer.id == self.pocketeerId) return pocketeer;
          return _.omit(pocketeer, 'customData');
        });
        if (!opts.includeExtendedInfo) {
          wallet = _.omit(wallet, walletExtendedKeys);
          wallet.pocketeers = _.map(wallet.pocketeers, function(pocketeer) {
            return _.omit(pocketeer, pocketeerExtendedKeys);
          });
        }
 ...
 * @param {Object} data
 * @param {Object} opts
 * @param {Boolean} opts.isGlobal - If true, the notification is not issued on behalf of any particular pocketeer (defaults to false)
 */
WalletService.prototype._notify = function(type, data, opts, cb) {
  ...
  var pocketeerId = self.pocketeerId || data.pocketeerId;
  ...
  var notification = Model.Notification.create({
    ...
    creatorId: opts.isGlobal ? null : pocketeerId,
    walletId: walletId,
  });
...
WalletService.prototype._addPocketeerToWallet = function(wallet, opts, cb) {
  var self = this;

  var pocketeer = Model.Pocketeer.create({
    coin: wallet.coin,
    name: opts.name,
    pocketeerIndex: wallet.pocketeers.length,
    xPubKey: opts.xPubKey,
    requestPubKey: opts.requestPubKey,
    signature: opts.pocketeerSignature,
    customData: opts.customData,
    derivationStrategy: wallet.derivationStrategy,
  });

  self.storage.fetchPocketeerLookup(pocketeer.id, function(err, res) {
    if (err) return cb(err);
    if (res) return cb(Errors.POCKETEER_REGISTERED);
    ...
    wallet.addPocketeer(pocketeer);
    self.storage.storeWalletAndUpdatePocketeersLookup(wallet, function(err) {
      if (err) return cb(err);

      async.series([

        function(next) {
          self._notify('NewPocketeer', {
            walletId: opts.walletId,
            pocketeerId: pocketeer.id,
            pocketeerName: pocketeer.name,
          }, next);
        },
        function(next) {
...
WalletService.prototype._addKeyToPocketeer = function(wallet, pocketeer, opts, cb) {
  var self = this;
  wallet.addPocketeerRequestKey(pocketeer.pocketeerId, opts.requestPubKey, opts.signature, opts.restrictions, opts.name);
  self.storage.storeWalletAndUpdatePocketeersLookup(wallet, function(err) {
    if (err) return cb(err);
...
/**
 * Adds access to a given pocketeer
 *
 * @param {Object} opts
 * @param {string} opts.pocketeerId - The pocketeer id
 * @param {string} opts.requestPubKey - Public Key used to check requests from this pocketeer.
 * @param {string} opts.pocketeerSignature - S(requestPubKey). Used by other pocketeers to verify the that the pocketeer is himself (signed with REQUEST_KEY_AUTH)
...
WalletService.prototype.addAccess = function(opts, cb) {
  ...
  if (!checkRequired(opts, ['pocketeerId', 'requestPubKey', 'signature'], cb)) return;

  self.storage.fetchPocketeerLookup(opts.pocketeerId, function(err, pocketeer) {
    if (err) return cb(err);
    if (!pocketeer) return cb(Errors.NOT_AUTHORIZED);
    self.storage.fetchWallet(pocketeer.walletId, function(err, wallet) {
      if (err) return cb(err);
      if (!wallet) return cb(Errors.NOT_AUTHORIZED);

      var xPubKey = _.find(wallet.pocketeers, {
        id: opts.pocketeerId
      }).xPubKey;
      ...
      if (pocketeer.requestPubKeys.length > Defaults.MAX_KEYS)
        return cb(Errors.TOO_MANY_KEYS);

      self._addKeyToPocketeer(wallet, pocketeer, opts, cb);
    });
  });
};
...
WalletService._getPocketeerHash = function(name, xPubKey, requestPubKey) {
 ...
 * @param {string} opts.name - The pocketeer name.
 * @param {string} opts.xPubKey - Extended Public Key for this pocketeer.
 * @param {string} opts.requestPubKey - Public Key used to check requests from this pocketeer.
 * @param {string} opts.pocketeerSignature - S(name|xPubKey|requestPubKey). Used by other pocketeers to verify that the pocketeer joining knows the wallet secret.
 * @param {string} opts.customData - (optional) Custom data for this pocketeer.
...
WalletService.prototype.joinWallet = function(opts, cb) {
  ...
  if (!checkRequired(opts, ['walletId', 'name', 'xPubKey', 'requestPubKey', 'pocketeerSignature'], cb)) return;

  if (_.isEmpty(opts.name))
    return cb(new ClientError('Invalid pocketeer name'));
  ...
  self._runLocked(cb, function(cb) {
    self.storage.fetchWallet(opts.walletId, function(err, wallet) {
      ...
      var hash = WalletService._getPocketeerHash(opts.name, opts.xPubKey, opts.requestPubKey);
      if (!self._verifySignature(hash, opts.pocketeerSignature, wallet.pubKey)) {
        return cb(new ClientError());
      }

      if (_.find(wallet.pocketeers, {
        xPubKey: opts.xPubKey
      })) return cb(Errors.POCKETEER_IN_WALLET);

      if (wallet.pocketeers.length == wallet.n) return cb(Errors.WALLET_FULL);

      self._addPocketeerToWallet(wallet, opts, cb);
    });
  });
};

/**
 * Save pocketeer preferences for the current wallet/pocketeer pair.
 * @param {Object} opts
...
WalletService.prototype.savePreferences = function(opts, cb) {
  ...
  self._runLocked(cb, function(cb) {
    self.storage.fetchPreferences(self.walletId, self.pocketeerId, function(err, oldPref) {
      if (err) return cb(err);
...
/**
 * Retrieves a preferences for the current wallet/pocketeer pair.
 * @param {Object} opts
...
WalletService.prototype.getPreferences = function(opts, cb) {
  ...
  self.storage.fetchPreferences(self.walletId, self.pocketeerId, function(err, preferences) {
    if (err) return cb(err);
...
/**
 * Verifies that a given message was actually sent by an authorized pocketeer.
 * @param {Object} opts
...
WalletService.prototype.verifyMessageSignature = function(opts, cb) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    var pocketeer = wallet.getPocketeer(self.pocketeerId);

    var isValid = !!self._getSigningKey(opts.message, opts.signature, pocketeer.requestPubKeys);
...
WalletService.prototype._canCreateTx = function(cb) {
  var self = this;
  self.storage.fetchLastTxs(self.walletId, self.pocketeerId, 5 + Defaults.BACKOFF_OFFSET, function(err, txs) {
    if (err) return cb(err);
...
/**
 * Publish an already created tx proposal so inputs are locked and other pocketeers in the wallet can see it.
 * @param {Object} opts
 * @param {string} opts.txProposalId - The tx id.
 * @param {string} opts.proposalSignature - S(raw tx). Used by other pocketeers to verify the proposal.
 */
  ...
  self._runLocked(cb, function(cb) {
    self.getWallet({}, function(err, wallet) {
      ...
      self.storage.fetchTx(self.walletId, opts.txProposalId, function(err, txp) {
        ...
        var signingKey = self._getSigningKey(raw, opts.proposalSignature, pocketeer.requestPubKeys);
        if (!signingKey) {
          return cb(new ClientError('Invalid proposal signature'));
        }

        // Save signature info for other pocketeers to check
  ...
  self._runLocked(cb, function(cb) {
    self.storage.fetchTxNote(self.walletId, opts.txid, function(err, note) {
      ...
      if (!note) {
        ...
        note.edit(opts.body, self.pocketeerId);
      }
...
WalletService.prototype.getRemainingDeleteLockTime = function(txp) {
  ...
  if (txp.creatorId !== this.pocketeerId)
    return lockTimeRemaining;

  // has other approvers? need to wait
  var approvers = txp.getApprovers();
  if (approvers.length > 1 || (approvers.length == 1 && approvers[0] !== this.pocketeerId))
    return lockTimeRemaining;
 ...
 * @param {string} opts.signatures - The signatures of the inputs of this tx for this pocketeer (in apperance order)
 */
WalletService.prototype.signTx = function(opts, cb) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    self.getTx({
      ...
      if (action) return cb(Errors.POCKETEER_VOTED);
      if (!txp.isPending()) return cb(Errors.TX_NOT_PENDING);

      var pocketeer = wallet.getPocketeer(self.pocketeerId);

      try {
        if (!txp.sign(self.pocketeerId, opts.signatures, pocketeer.xPubKey)) {
          log.warn('Error signing transaction (BAD_SIGNATURES)');
          log.warn('Wallet id:', self.walletId);
          log.warn('Pocketeer id:', self.pocketeerId);
 ...
 * @param {Object} opts
 * @param {string} opts.txProposalId - The identifier of the transaction.
 * @param {string} [opts.reason] - A message to other pocketeers explaining the rejection.
 */
WalletService.prototype.rejectTx = function(opts, cb) {
  ...
  self.getTx({
    ...
    txp.reject(self.pocketeerId, opts.reason);
    ...
    return _.map(txs, function(tx) {
      ...
      if (proposal) {
        ...
        newTx.actions = _.map(proposal.actions, function(action) {
          return _.pick(action, ['createdOn', 'type', 'pocketeerId', 'pocketeerName', 'comment']);
        });
 ...
 *
 * @param {Object} opts
 * @param {Boolean} opts.includePocketeerBranches (defaults to false)
 */
      ...
      self.storage.clearTxHistoryCache(self.walletId, function() {
        self.storage.storeWallet(wallet, function(err) {
          ...
          _.each([false, true], function(isChange) {
            ...
            if (opts.includePocketeerBranches) {
              _.each(wallet.pocketeers, function(pocketeer) {
                if (pocketeer.addressManager) {
                  derivators.push({
                    derive: _.bind(pocketeer.createAddress, pocketeer, wallet, isChange),
                    rewind: _.bind(pocketeer.addressManager.rewindIndex, pocketeer.addressManager, isChange),
                  });
                }
              });
            }
...
/**
 * Subscribe this pocketeer to the Push Notifications service using the specified token.
 * @param {Object} opts
...
/**
 * Unsubscribe this pocketeer to the Push Notifications service using the specified token.
 * @param {Object} opts
 * @param {string} opts.token - The token representing the app/device.
 */
WalletService.prototype.pushNotificationsUnsubscribe = function(opts, cb) {
  ...
  self.storage.removePushNotificationSub(self.pocketeerId, opts.token, cb);
};

/**
 * Subscribe this pocketeer to the specified tx to get a notification when the tx confirms.
 * @param {Object} opts
...
WalletService.prototype.txConfirmationUnsubscribe = function(opts, cb) {
  ...
  self.storage.removeTxConfirmationSub(self.pocketeerId, opts.txid, cb);
};
