      ...
      async.waterfall([
        ...
        function(contents, next) {
          async.map(recipientsList, function(recipient, next) {
            ...
            self.storage.fetchPushNotificationSubs(recipient.pocketeerId, function(err, subs) {
              ...
              var notifications = _.map(subs, function(sub) {
                return {
                  ...
                  data: {
                    ...
                    pocketeerId: sjcl.codec.hex.fromBits(sjcl.hash.sha256.hash(recipient.pocketeerId))
                  },
                };
              });
  ...
  self.storage.fetchWallet(notification.walletId, function(err, wallet) {
    ...
    self.storage.fetchPreferences(notification.walletId, null, function(err, preferences) {
      ...
      var recipientPreferences = _.compact(_.map(preferences, function(p) {
        ...
        return {
          pocketeerId: p.pocketeerId,
          language: p.language,
          unit: unit || p.unit || self.defaultUnit,
        };
      }));

      recipientPreferences = _.indexBy(recipientPreferences, 'pocketeerId');

      var recipientsList = _.compact(_.map(wallet.pocketeers, function(pocketeer) {
        if ((pocketeer.id == notification.creatorId && notificationType.notifyCreatorOnly) ||
          (pocketeer.id != notification.creatorId && !notificationType.notifyCreatorOnly)) {
          var p = recipientPreferences[pocketeer.id] || {};
          return {
...
PushNotificationsService.prototype._getDataForTemplate = function(notification, recipient, cb) {
  ...
  self.storage.fetchWallet(notification.walletId, function(err, wallet) {
    ...
    var pocketeer = _.find(wallet.pocketeers, {
      id: notification.creatorId
    });

    if (pocketeer) {
      data.pocketeerId = pocketeer.id;
      data.pocketeerName = pocketeer.name;
    }

    if (notification.type == 'TxProposalFinallyRejected' && data.rejectedBy) {
      var rejectors = _.map(data.rejectedBy, function(pocketeerId) {
        return _.find(wallet.pocketeers, {
          id: pocketeerId
        }).name
      });
