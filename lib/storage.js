...
var collections = {
  ...
  POCKETEERS_LOOKUP: 'pocketeers_lookup',
...
Storage.prototype._createIndexes = function() {
  ...
  this.db.collection(collections.POCKETEERS_LOOKUP).createIndex({
    pocketeerId: 1
  });
...
Storage.prototype.storeWalletAndUpdatePocketeersLookup = function(wallet, cb) {
  var self = this;

  var pocketeerLookups = _.map(wallet.pocketeers, function(pocketeer) {
    $.checkState(pocketeer.requestPubKeys);
    return {
      pocketeerId: pocketeer.id,
      walletId: wallet.id,
      requestPubKeys: pocketeer.requestPubKeys,
    };
  });

  this.db.collection(collections.POCKETEERS_LOOKUP).remove({
    walletId: wallet.id
  }, {
    w: 1
  }, function(err) {
    if (err) return cb(err);
    self.db.collection(collections.POCKETEERS_LOOKUP).insert(pocketeerLookups, {
      w: 1
    }, function(err) {
      if (err) return cb(err);
...
Storage.prototype.fetchPocketeerLookup = function(pocketeerId, cb) {

  this.db.collection(collections.POCKETEERS_LOOKUP).findOne({
...
Storage.prototype._completeTxData = function(walletId, txs, cb) {
  ...
  self.fetchWallet(walletId, function(err, wallet) {
    ...
    _.each([].concat(txs), function(tx) {
      ...
      tx.creatorName = wallet.getPocketeer(tx.creatorId).name;
      _.each(tx.actions, function(action) {
        action.pocketeerName = wallet.getPocketeer(action.pocketeerId).name;
      });
...
Storage.prototype.fetchPreferences = function(walletId, pocketeerId, cb) {
...
Storage.prototype.storePreferences = function(preferences, cb) {
  this.db.collection(collections.PREFERENCES).update({
    ...
    pocketeerId: preferences.pocketeerId,
  }, preferences, {
    w: 1,
    upsert: true,
  }, cb);
...
Storage.prototype._completeTxNotesData = function(walletId, notes, cb) {
  ...
  self.fetchWallet(walletId, function(err, wallet) {
    ...
    _.each([].concat(notes), function(note) {
      note.editedByName = wallet.getPocketeer(note.editedBy).name;
    });
...
Storage.prototype.getSession = function(pocketeerId, cb) {
  var self = this;
...
Storage.prototype.storeSession = function(session, cb) {
  this.db.collection(collections.SESSIONS).update({
    pocketeerId: session.pocketeerId,
  }, session.toObject(), {
    w: 1,
    upsert: true,
  }, cb);
...
Storage.prototype.fetchPushNotificationSubs = function(pocketeerId, cb) {
...
Storage.prototype.storePushNotificationSub = function(pushNotificationSub, cb) {
  this.db.collection(collections.PUSH_NOTIFICATION_SUBS).update({
    pocketeerId: pushNotificationSub.pocketeerId,
...
Storage.prototype.removePushNotificationSub = function(pocketeerId, token, cb) {
...
Storage.prototype.fetchActiveTxConfirmationSubs = function(pocketeerId, cb) {
  var filter = {
    isActive: true
  };
  if (pocketeerId) filter.pocketeerId = pocketeerId;
...
Storage.prototype.storeTxConfirmationSub = function(txConfirmationSub, cb) {
  this.db.collection(collections.TX_CONFIRMATION_SUBS).update({
    pocketeerId: txConfirmationSub.pocketeerId,
...
Storage.prototype.removeTxConfirmationSub = function(pocketeerId, txid, cb) {
