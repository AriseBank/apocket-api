...
ExpressApp.prototype.start = function(opts, cb) {
  ...
  if (opts.disableLogs) {
    ...
    morgan.token('pocketeerId', function getId(req) {
      return req.pocketeerId
    });

    var logFormat = ':remote-addr :date[iso] ":method :url" :status :res[content-length] :response-time ":user-agent" :walletId :pocketeerId';
    var logOpts = {
  ...
  function getServerWithAuth(req, res, opts, cb) {
    ...
    var auth = {
      pocketeerId: credentials.pocketeerId,
    ...
    WalletService.getInstanceWithAuth(auth, function(err, server) {
      ...
      if (opts.onlySupportStaff && !server.pocketeerIsSupportStaff) {
        return returnError(new WalletService.ClientError({
          code: 'NOT_AUTHORIZED'
        }), res, req);
      }
      ...
      req.pocketeerId = server.pocketeerId;
  ...
  router.put('/v1/pocketeers/:id/', function(req, res) {
    req.body.pocketeerId = req.params['id'];
    var server;
    try {
      server = getServer(req, res);
    } catch (ex) {
  ...
  // DEPRECATED
  router.post('/v1/wallets/:id/pocketeers/', function(req, res) {
    logDeprecated(req);
  ...
  router.post('/v2/wallets/:id/pocketeers/', function(req, res) {
    req.body.walletId = req.params['id'];
    var server;
    try {
      server = getServer(req, res);
  ...
  this.app.use(opts.basePath || '/apws/api', router);
