...
AddressManager.create = function(opts) {
  ...
  x.pocketeerIndex = _.isNumber(opts.pocketeerIndex) ? opts.pocketeerIndex : Constants.BIP45_SHARED_INDEX;
...
AddressManager.fromObj = function(obj) {
  ...
  x.pocketeerIndex = obj.pocketeerIndex;

  return x;
};

AddressManager.supportsPocketeerBranches = function(derivationStrategy) {
...
AddressManager.prototype.getCurrentAddressPath = function(isChange) {
  return 'm/' +
    (this.derivationStrategy == Constants.DERIVATION_STRATEGIES.BIP45 ? this.pocketeerIndex + '/' : '') +
    (isChange ? 1 : 0) + '/' +
