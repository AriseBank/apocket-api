 ...
 * getActors
 *
 * @return {String[]} pocketeerIds that performed actions in this proposal (accept / reject)
 */
TxProposal.prototype.getActors = function() {
  return _.pluck(this.actions, 'pocketeerId');
};
 ...
 * getApprovers
 *
 * @return {String[]} pocketeerIds that approved the tx proposal (accept)
 */
 ...
 * getActionBy
 *
 * @param {String} pocketeerId
 * @return {Object} type / createdOn
 */
TxProposal.prototype.getActionBy = function(pocketeerId) {
  return _.find(this.actions, {
    pocketeerId: pocketeerId
  });
};

TxProposal.prototype.addAction = function(pocketeerId, type, comment, signatures, xpub) {
...
TxProposal.prototype.sign = function(pocketeerId, signatures, xpub) {
  try {
    // Tests signatures are OK
    var tx = this.getBitcoreTx();
    this._addSignaturesToBitcoreTx(tx, signatures, xpub);

    this.addAction(pocketeerId, 'accept', null, signatures, xpub);
...
TxProposal.prototype.reject = function(pocketeerId, reason) {
  this.addAction(pocketeerId, 'reject', reason);
};
