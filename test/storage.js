...
describe('Storage', function() {
  ...
  describe('Pocketeer lookup', function() {
    it('should correctly store and fetch pocketeer lookup', function(done) {
      var wallet = Model.Wallet.create({
        id: '123',
        name: 'my wallet',
        m: 2,
      ...
      should.exist(wallet);
      storage.storeWalletAndUpdatePocketeersLookup(wallet, function(err) {
        should.not.exist(err);
        storage.fetchPocketeerLookup(wallet.pocketeers[1].id, function(err, lookup) {
          should.not.exist(err);
          should.exist(lookup);
    ...
    it('should not return error if pocketeer not found', function(done) {
      storage.fetchPocketeerLookup('2', function(err, lookup) {
        should.not.exist(err);
  ...
  describe('Transaction proposals', function() {
      ...
      storage.storeWalletAndUpdatePocketeersLookup(wallet, function(err) {
        ...
        proposals = _.map(_.range(4), function(i) {
          var tx = Model.TxProposal.create({
            ...
            feePerKb: 100e2,
            creatorId: wallet.pocketeers[0].id,
          });
          if (i % 2 == 0) {
            tx.status = 'pending';
    ...
    it('should fetch tx', function(done) {
      storage.fetchTx('123', proposals[0].id, function(err, tx) {
        ...
        tx.creatorName.should.equal('pocketeer 0');
        done();
      });
    });
