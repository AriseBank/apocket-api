...
describe('Wallet service', function() {
  ...
  describe('#getServiceVersion', function() {
    it('should get version from package', function() {
      WalletService.getServiceVersion().should.equal('apws-' + require('../../package').version);
    });
  });
  ...
  describe('#getInstanceWithAuth', function() {
    ...
    it('should get server instance for existing pocketeer', function(done) {
      helpers.createAndJoinWallet(1, 2, function(s, wallet) {
        var xpriv = TestData.pocketeers[0].xPrivKey;
        var priv = TestData.pocketeers[0].privKey_1H_0;

        var sig = helpers.signMessage('hello world', priv);

        WalletService.getInstanceWithAuth({
          pocketeerId: wallet.pocketeers[0].id,
          message: 'hello world',
          signature: sig,
          ...
          server.pocketeerId.should.equal(wallet.pocketeers[0].id);
    ...
    it('should fail when requesting for non-existent pocketeer', function(done) {
      var message = 'hello world';
      var opts = {
        pocketeerId: 'dummy',
        message: message,
        signature: helpers.signMessage(message, TestData.pocketeers[0].privKey_1H_0),
      };
      WalletService.getInstanceWithAuth(opts, function(err, server) {
        err.code.should.equal('NOT_AUTHORIZED');
        err.message.should.contain('Pocketeer not found');
        done();
      });
    });
    ...
    it('should get server instance for support staff', function(done) {
      helpers.createAndJoinWallet(1, 1, function(s, wallet) {
        ...
        s.storage.db.collection(collections.POCKETEERS_LOOKUP).update({
          pocketeerId: wallet.pocketeers[0].id
        }, {
          $set: {
            isSupportStaff: true
  ...
  describe('Session management (#login, #logout, #authenticate)', function() {
    ...
    it('should get a new session & authenticate', function(done) {
      WalletService.getInstanceWithAuth({
        pocketeerId: server.pocketeerId,
        session: 'dummy',
        ...
        server.login({}, function(err, token) {
          ...
          WalletService.getInstanceWithAuth({
            ...
            server2.pocketeerId.should.equal(server.pocketeerId);
    ...
    it('should check m-n combination', function(done) {
      ...
      async.each(pairs, function(pair, cb) {
        ...
        server.createWallet(opts, function(err) {
          if (!pair.valid) {
            should.exist(err);
            err.message.should.equal('Invalid combination of required pocketeers / total pocketeers');
          } else {
            should.not.exist(err);
          }
          return cb();
    ...
    describe('New clients', function() {
      ...
      it('should join existing wallet', function(done) {
        var pocketeerOpts = helpers.getSignedPocketeerOpts({
          walletId: walletId,
          name: 'me',
          xPubKey: TestData.pocketeers[0].xPubKey_44H_0H_0H,
          requestPubKey: TestData.pocketeers[0].pubKey_1H_0,
          customData: 'dummy custom data',
        });
        server.joinWallet(pocketeerOpts, function(err, result) {
          should.not.exist(err);
          var pocketeerId = result.pocketeerId;
          helpers.getAuthServer(pocketeerId, function(server) {
            server.getWallet({}, function(err, wallet) {
              wallet.id.should.equal(walletId);
              wallet.pocketeers.length.should.equal(1);
              var pocketeer = wallet.pocketeers[0];
              pocketeer.name.should.equal('me');
              pocketeer.id.should.equal(pocketeerId);
              pocketeer.customData.should.equal('dummy custom data');
              server.getNotifications({}, function(err, notifications) {
                ...
                notif.data.pocketeerId.should.equal(pocketeerId);
                notif.data.pocketeerName.should.equal('me');
      ...
      it('should fail to join non-existent wallet', function(done) {
        var pocketeerOpts = {
          ...
          xPubKey: 'dummy',
          requestPubKey: 'dummy',
          pocketeerSignature: 'dummy',
        };
        server.joinWallet(pocketeerOpts, function(err) {
          should.exist(err);
          done();
        });
      });
      ...
      it('should fail to join full wallet', function(done) {
        helpers.createAndJoinWallet(1, 1, function(s, wallet) {
          var pocketeerOpts = helpers.getSignedPocketeerOpts({
            walletId: wallet.id,
            name: 'me',
            xPubKey: TestData.pocketeers[1].xPubKey_44H_0H_0H,
            requestPubKey: TestData.pocketeers[1].pubKey_1H_0,
          });
          server.joinWallet(pocketeerOpts, function(err) {
            should.exist(err);
      ...
      it('should return pocketeer in wallet error before full wallet', function(done) {
        helpers.createAndJoinWallet(1, 1, function(s, wallet) {
          var pocketeerOpts = helpers.getSignedPocketeerOpts({
            walletId: wallet.id,
            name: 'me',
          ...
          server.joinWallet(pocketeerOpts, function(err) {
            should.exist(err);
            err.code.should.equal('POCKETEER_IN_WALLET');
            done();
          });
        });
      });
      ...
      it('should fail to re-join wallet', function(done) {
        ...
        server.joinWallet(pocketeerOpts, function(err) {
          ...
          server.joinWallet(pocketeerOpts, function(err) {
            should.exist(err);
            err.code.should.equal('POCKETEER_IN_WALLET');
            err.message.should.equal('Pocketeer already in wallet');
            done();
          });
        });
      });
      ...
      it('should be able to get wallet info without actually joining', function(done) {
        ...
        server.joinWallet(pocketeerOpts, function(err, result) {
          should.not.exist(err);
          should.exist(result);
          should.not.exist(result.pocketeerId);
          result.wallet.id.should.equal(walletId);
          result.wallet.m.should.equal(1);
          result.wallet.n.should.equal(2);
          result.wallet.pocketeers.should.be.empty;
          server.storage.fetchWallet(walletId, function(err, wallet) {
            should.not.exist(err);
            wallet.id.should.equal(walletId);
            wallet.pocketeers.should.be.empty;
            done();
          });
        });
      });
      ...
      it('should fail to join two wallets with same xPubKey', function(done) {
        ...
        server.joinWallet(pocketeerOpts, function(err) {
          ...
          server.createWallet(walletOpts, function(err, walletId) {
            should.not.exist(err);
            pocketeerOpts = helpers.getSignedPocketeerOpts({
              walletId: walletId,
              name: 'me',
              xPubKey: TestData.pocketeers[0].xPubKey_44H_0H_0H,
              requestPubKey: TestData.pocketeers[0].pubKey_1H_0,
            ...
            server.joinWallet(pocketeerOpts, function(err) {
              should.exist(err);
              err.code.should.equal('POCKETEER_REGISTERED');
              err.message.should.equal('Pocketeer ID already registered on server');
              done();
            });
          });
        });
      ...
      it('should fail to join with null signature', function(done) {
        ...
        server.joinWallet(pocketeerOpts, function(err) {
          should.exist(err);
          err.message.should.contain('argument pocketeerSignature missing');
          done();
        });
      });
      ...
      it('should set pkr and status = complete on last pocketeer joining (2-3)', function(done) {
      ...
      it('should fail to join new wallet from legacy client', function(done) {
        ...
        server.createWallet(walletOpts, function(err, walletId) {
          ...
          var pocketeerOpts = helpers.getSignedPocketeerOpts({
            walletId: walletId,
            name: 'me',
            xPubKey: TestData.pocketeers[0].xPubKey_45H,
            requestPubKey: TestData.pocketeers[0].pubKey_1H_0,
            supportBIP44AndP2PKH: false,
          });
          server.joinWallet(pocketeerOpts, function(err, result) {
      ...
      helpers.createAndJoinWallet(1, 1, function(s, w) {
        ...
        helpers.stubUtxos(server, wallet, [1, 2], function() {
          ...
          async.eachSeries(_.range(2), function(i, next) {
            helpers.createAndPublishTx(server, txOpts, TestData.pocketeers[0].privKey_1H_0, function() {
              next();
            });
          }, done);
        });
        ...
        function(next) {
          helpers.createAndJoinWallet(1, 1, {
            ...
            helpers.stubUtxos(server2, wallet2, [1, 2, 3], function() {
              ...
              async.eachSeries(_.range(2), function(i, next) {
                helpers.createAndPublishTx(server2, txOpts, TestData.pocketeers[1].privKey_1H_0, function() {
                  next();
                });
              }, next);
            });
  ...
  describe('#getStatus', function() {
    ...
    it('should get status', function(done) {
      server.getStatus({}, function(err, status) {
        ...
        should.exist(status.wallet.pocketeers);
        status.wallet.pocketeers.length.should.equal(2);
        ...
        _.each(status.wallet.pocketeers, function(pocketeer) {
          should.not.exist(pocketeer.xPubKey);
          should.not.exist(pocketeer.requestPubKey);
          should.not.exist(pocketeer.signature);
          should.not.exist(pocketeer.requestPubKey);
          should.not.exist(pocketeer.addressManager);
          should.not.exist(pocketeer.customData);
        });
        done();
      });
    });
    it('should get status including extended info', function(done) {
      server.getStatus({
        ...
        should.exist(status.wallet.pocketeers[0].xPubKey);
        should.exist(status.wallet.pocketeers[0].requestPubKey);
        should.exist(status.wallet.pocketeers[0].signature);
        should.exist(status.wallet.pocketeers[0].requestPubKey);
        should.exist(status.wallet.pocketeers[0].customData);
        // Do not return other pocketeer's custom data
        _.each(_.rest(status.wallet.pocketeers), function(pocketeer) {
          should.not.exist(pocketeer.customData);
        });
        done();
      });
  ...
  describe('#verifyMessageSignature', function() {
    ...
    it('should fail to verify message signature for different pocketeer', function(done) {
      var message = 'hello world';
      var opts = {
        message: message,
        signature: helpers.signMessage(message, TestData.pocketeers[0].privKey_1H_0),
      };
      helpers.getAuthServer(wallet.pocketeers[1].id, function(server) {
        server.verifyMessageSignature(opts, function(err, isValid) {
          should.not.exist(err);
    ...
    describe('1-of-1 (BIP44 & P2PKH)', function() {
      beforeEach(function(done) {
        helpers.createAndJoinWallet(1, 1, function(s, w) {
          server = s;
          wallet = w;
          w.pocketeers[0].id.should.equal(TestData.pocketeers[0].id44btc);
          done();
        });
      });
  ...
  describe('Preferences', function() {
    ...
    it('should save preferences only for requesting pocketeer', function(done) {
      server.savePreferences({
        email: 'dummy@dummy.com'
      }, function(err) {
        should.not.exist(err);
        helpers.getAuthServer(wallet.pocketeers[1].id, function(server2) {
  ...
  describe('#getUtxos', function() {
    ...
    it('should not fail when getting UTXOs for wallet with 0 UTXOs and pending txps', function(done) {
      helpers.stubUtxos(server, wallet, [1, 1], function() {
        ...
        helpers.createAndPublishTx(server, txOpts, TestData.pocketeers[0].privKey_1H_0, function(txp) {
  ...
  describe('Multiple request Pub Keys', function() {
    ...
    var getAuthServer = function(pocketeerId, privKey, cb) {
      var msg = 'dummy';
      var sig = helpers.signMessage(msg, privKey);
      WalletService.getInstanceWithAuth({
        pocketeerId: pocketeerId,
        message: msg,
        signature: sig,
    ...
    beforeEach(function() {
      ...
      var xPrivKey = TestData.pocketeers[0].xPrivKey_44H_0H_0H;
      var requestPubKeyStr = requestPubKey.toString();
      var sig = helpers.signRequestPubKey(requestPubKeyStr, xPrivKey);

      var pocketeerId = Model.Pocketeer._xPubToPocketeerId('btc', TestData.pocketeers[0].xPubKey_44H_0H_0H);
      opts = {
        pocketeerId: pocketeerId,
    ...
    describe('#addAccess 1-1', function() {
      ...
      it('should be able to re-gain access from xPrivKey', function(done) {
        ws.addAccess(opts, function(err, res) {
          should.not.exist(err);
          res.wallet.pocketeers[0].requestPubKeys.length.should.equal(2);
          res.wallet.pocketeers[0].requestPubKeys[0].selfSigned.should.equal(true);

          server.getBalance(res.wallet.walletId, function(err, bal) {
            should.not.exist(err);
            bal.totalAmount.should.equal(1e8);
            getAuthServer(opts.pocketeerId, reqPrivKey, function(err, server2) {
      ...
      it('should fail to access with wrong privkey after gaining access', function(done) {
        ws.addAccess(opts, function(err, res) {
          ...
          server.getBalance(res.wallet.walletId, function(err, bal) {
            ...
            (getAuthServer(opts.pocketeerId, privKey, function(err, server2) {
              err.code.should.equal('NOT_AUTHORIZED');
              done();
            }));
          });
  ...
  describe('Wallet not complete tests', function() {
    it('should fail to create address when wallet is not complete', function(done) {
      ...
      server.createWallet(walletOpts, function(err, walletId) {
        ...
        server.joinWallet(pocketeerOpts, function(err, result) {
          should.not.exist(err);
          helpers.getAuthServer(result.pocketeerId, function(server) {
            server.createAddress({}, function(err, address) {
              should.not.exist(address);
              should.exist(err);
    ...
    it('should fail to create tx when wallet is not complete', function(done) {
      ...
      server.createWallet(walletOpts, function(err, walletId) {
        ...
        server.joinWallet(pocketeerOpts, function(err, result) {
          should.not.exist(err);
          helpers.getAuthServer(result.pocketeerId, function(server, wallet) {
            var txOpts = {
              outputs: [{
                toAddress: '18PzpUFkFZE8zKWUPvfykkTxmB9oMR8qP7',
                amount: 0.8e8
    ...
    describe('#createTx ' + coin, function() {
      ...
      describe('Tx proposal creation & publishing ' + coin, function() {
          ...
          it('should return already published tx if same foreign ID is specified and tx already published', function(done) {
            helpers.stubUtxos(server, wallet, [2, 2, 2], function() {
              ...
              server.createTx(txOpts, function(err, tx) {
                ...
                should.exist(tx);
                tx.id.should.equal('123');
                var publishOpts = helpers.getProposalSignatureOpts(tx, TestData.pocketeers[0].privKey_1H_0);
        ...
        describe('Publishing', function() {
          it('should be able to publish a temporary tx proposal', function(done) {
            helpers.stubUtxos(server, wallet, [1, 2], function() {
              ...
              server.createTx(txOpts, function(err, txp) {
                should.not.exist(err);
                should.exist(txp);
                var publishOpts = helpers.getProposalSignatureOpts(txp, TestData.pocketeers[0].privKey_1H_0);
                ...
                server.getNotifications({}, function(err, notifications) {
                  ...
                  server.publishTx(publishOpts, function(err) {
                    ...
                    server.getNotifications({}, function(err, notifications) {
                      ...
                      n.data.creatorId.should.equal(server.pocketeerId);
                      done();
                    });
                  });
                });
          ...
          it('should fail to publish tx proposal not signed by the creator', function(done) {
            helpers.stubUtxos(server, wallet, [1, 2], function() {
              ...
              server.createTx(txOpts, function(err, txp) {
                ...
                var publishOpts = {
                  txProposalId: txp.id,
                  proposalSignature: helpers.signMessage(txp.getRawTx(), TestData.pocketeers[1].privKey_1H_0),
                }
          ...
          it('should fail to publish a temporary tx proposal if utxos are locked by other pending proposals', function(done) {
            ...
            async.waterfall([
              ...
              function(txp, next) {
                ...
                should.exist(txp1);
                should.exist(txp2);
                var publishOpts = helpers.getProposalSignatureOpts(txp1, TestData.pocketeers[0].privKey_1H_0);
                server.publishTx(publishOpts, next);
              },
              function(txp, next) {
                var publishOpts = helpers.getProposalSignatureOpts(txp2, TestData.pocketeers[0].privKey_1H_0);
                server.publishTx(publishOpts, function(err) {
                  should.exist(err);
              ...
              function(txp3, next) {
                should.exist(txp3);
                var publishOpts = helpers.getProposalSignatureOpts(txp3, TestData.pocketeers[0].privKey_1H_0);
                server.publishTx(publishOpts, next);
              },
              function(txp, next) {
                server.getPendingTxs({}, function(err, txs) {
          ...
          it('should fail to publish a temporary tx proposal if utxos are already spent', function(done) {
            ...
            async.waterfall([
              ...
              function(txp, next) {
                // Sign & Broadcast txp1
                var signatures = helpers.clientSign(txp, TestData.pocketeers[0].xPrivKey_44H_0H_0H);
                server.signTx({
                  txProposalId: txp.id,
        ...
        it('should accept a tx proposal signed with a custom key', function(done) {
          ...
          var accessOpts = {
            pocketeerId: TestData.pocketeers[0][idKey],
  ...
  describe('Transaction notes', function(done) {
    ...
    it('should edit a note for an arbitrary txid', function(done) {
      server.editTxNote({
        ...
        note.editedBy.should.equal(server.pocketeerId);
        note.editedByName.should.equal('pocketeer 1');
          ...
          server.signTx({
            ...
            server.editTxNote({
              ...
              server.getTx({
                ...
                txp.note.editedBy.should.equal(server.pocketeerId);
                done();
              });
            });
          });
    ...
    it('should share notes between pocketeers', function(done) {
      server.editTxNote({
        txid: '123',
        body: 'note body'
      }, function(err) {
      ...
      helpers.createAddresses(server, wallet, 1, 1, function(mainAddresses, changeAddress) {
        ...
        server.editTxNote({
          ...
          server.getTxHistory({}, function(err, txs) {
            ...
            tx.note.editedBy.should.equal(server.pocketeerId);
        ...
        server.rejectTx({
          ...
          server.getPendingTxs({}, function(err, txs) {
            ...
            server.getTx({
              ...
              actors[0].should.equal(wallet.pocketeers[0].id);
              var action = tx.getActionBy(wallet.pocketeers[0].id);
  ...
  describe('#signTx', function() {
    describe('1-of-1 (BIP44 & P2PKH)', function() {
      ...
      it('should sign a TX with multiple inputs, different paths, and return raw', function(done) {
        ...
        server.getPendingTxs({}, function(err, txs) {
          var tx = txs[0];
          tx.id.should.equal(txid);
          var signatures = helpers.clientSign(tx, TestData.pocketeers[0].xPrivKey_44H_0H_0H);
    ...
    describe('Multisig', function() {
        ...
        server.getPendingTxs({}, function(err, txs) {
          ...
          server.signTx({
            ...
            server.getPendingTxs({}, function(err, txs) {
              ...
              actors.length.should.equal(1);
              actors[0].should.equal(wallet.pocketeers[0].id);
              tx.getActionBy(wallet.pocketeers[0].id).type.should.equal('accept');
      ...
      it('should fail to sign with a xpriv from other pocketeer', function(done) {
        server.getPendingTxs({}, function(err, txs) {
          var tx = txs[0];
          tx.id.should.equal(txid);
          var signatures = helpers.clientSign(tx, TestData.pocketeers[1].xPrivKey_44H_0H_0H);
          server.signTx({
            txProposalId: txid,
            signatures: signatures,
          }, function(err) {
      ...
      it('should fail on wrong number of invalid signatures', function(done) {
        server.getPendingTxs({}, function(err, txs) {
          ...
          var signatures = _.take(helpers.clientSign(tx, TestData.pocketeers[0].xPrivKey_44H_0H_0H), tx.inputs.length - 1);
          server.signTx({
            txProposalId: txid,
            signatures: signatures,
          }, function(err) {
        ...
        server.getPendingTxs({}, function(err, txs) {
          ...
          server.signTx({
            ...
            server.rejectTx({
              ...
              err.code.should.contain('POCKETEER_VOTED');
              done();
            });
          });
        });
      ...
      it('should fail to sign a non-pending TX', function(done) {
        async.waterfall([
          ...
          function(next) {
            helpers.getAuthServer(wallet.pocketeers[2].id, function(server) {
              server.getTx({
                txProposalId: txid
              }, function(err, tx) {
                should.not.exist(err);
                var signatures = helpers.clientSign(tx, TestData.pocketeers[2].xPrivKey_44H_0H_0H);
                server.signTx({
                  txProposalId: txid,
                  signatures: signatures,
                }, function(err) {
  ...
  describe('Tx proposal workflow', function() {
    ...
    it('other pocketeers should see pending proposal created by one pocketeer', function(done) {
      var txOpts = {
        outputs: [{
          toAddress: '18PzpUFkFZE8zKWUPvfykkTxmB9oMR8qP7',
          amount: 10e8
      ...
      helpers.createAndPublishTx(server, txOpts, TestData.pocketeers[0].privKey_1H_0, function(txp) {
        should.exist(txp);
        helpers.getAuthServer(wallet.pocketeers[1].id, function(server2, wallet) {
      ...
      async.waterfall([
        ...
        function(next) {
          server.getPendingTxs({}, function(err, txps) {
            ...
            var action = txp.getActionBy(wallet.pocketeers[0].id);
        ...
        function(txp, next) {
          helpers.getAuthServer(wallet.pocketeers[1].id, function(server, wallet) {
            var signatures = helpers.clientSign(txp, TestData.pocketeers[1].xPrivKey_44H_0H_0H);
            server.signTx({
              txProposalId: txpId,
              signatures: signatures,
            }, function(err) {
        ...
        function(next) {
          server.getPendingTxs({}, function(err, txps) {
            ...
            server.getNotifications({}, function(err, notifications) {
              ...
              last.creatorId.should.equal(wallet.pocketeers[1].id);
  ...
  describe('#getNotifications', function() {
    ...
    it('should pull all notifications', function(done) {
      server.getNotifications({}, function(err, notifications) {
        ...
        types.should.deep.equal(['NewPocketeer', 'NewAddress', 'NewAddress', 'NewTxProposal', 'NewTxProposal', 'NewTxProposal']);
        ...
        creators[0].should.equal(wallet.pocketeers[0].id);
        done();
      });
    });
    ...
    it('should contain walletId & creatorId on NewPocketeer', function(done) {
      server.getNotifications({}, function(err, notifications) {
        should.not.exist(err);
        var newPocketeer = notifications[0];
        newPocketeer.type.should.equal('NewPocketeer');
        newPocketeer.walletId.should.equal(wallet.id);
        newPocketeer.creatorId.should.equal(wallet.pocketeers[0].id);
        done();
      });
    });
  ...
  describe('#removePendingTx', function() {
    ...
    it('should not allow non-creator pocketeer to remove an unsigned TX ', function(done) {
      helpers.getAuthServer(wallet.pocketeers[1].id, function(server2) {
    ...
    it('should not allow creator pocketeer to remove a TX signed by other pocketeer, in less than 24hrs', function(done) {
      helpers.getAuthServer(wallet.pocketeers[1].id, function(server2) {
        var signatures = helpers.clientSign(txp, TestData.pocketeers[1].xPrivKey_44H_0H_0H);
        server2.signTx({
          txProposalId: txp.id,
    ...
    it('should allow creator pocketeer to remove a TX rejected by other pocketeer, in less than 24hrs', function(done) {
      helpers.getAuthServer(wallet.pocketeers[1].id, function(server2) {
        var signatures = helpers.clientSign(txp, TestData.pocketeers[1].xPrivKey_44H_0H_0H);
        server2.rejectTx({
          txProposalId: txp.id,
    ...
    it('should allow creator pocketeer to remove a TX signed by other pocketeer, after 24hrs', function(done) {
      helpers.getAuthServer(wallet.pocketeers[1].id, function(server2) {
        var signatures = helpers.clientSign(txp, TestData.pocketeers[1].xPrivKey_44H_0H_0H);
        server2.signTx({
          txProposalId: txp.id,
    ...
    it('should allow other pocketeer to remove a TX signed, after 24hrs', function(done) {
      helpers.getAuthServer(wallet.pocketeers[1].id, function(server2) {
        var signatures = helpers.clientSign(txp, TestData.pocketeers[1].xPrivKey_44H_0H_0H);
        server2.signTx({
          txProposalId: txp.id,
          ...
          server.signTx({
            ...
            server.broadcastTx({
              ...
              server.getTxHistory({}, function(err, txs) {
                ...
                tx.actions[0].pocketeerName.should.equal('pocketeer 1');
                tx.outputs[0].address.should.equal(external);
  ...
  describe('#scan', function() {
    ...
    describe('shared wallet (BIP45)', function() {
      ...
      it('should scan main addresses & pocketeer addresses', function(done) {
        helpers.stubAddressActivity(
          ['39AA1Y2VvPJhV3RFbc7cKbUax1WgkPwweR', // m/2147483647/0/0
            '3MzGaz4KKX66w8ShKaR536ZqzVvREBqqYu', // m/2147483647/1/0
    ...
    it('should start multiple asynchronous scans for different wallets', function(done) {
      ...
      server2.createWallet(opts, function(err, walletId) {
        ...
        var pocketeerOpts = helpers.getSignedPocketeerOpts({
          walletId: walletId,
          name: 'pocketeer 1',
          xPubKey: TestData.pocketeers[3].xPubKey_45H,
          requestPubKey: TestData.pocketeers[3].pubKey_1H_0,
        });
        server.joinWallet(pocketeerOpts, function(err, result) {
          should.not.exist(err);
          helpers.getAuthServer(result.pocketeerId, function(server2) {
            server.startScan({}, function(err) {
              should.not.exist(err);
  ...
  describe('Push notifications', function() {
    ...
    it('should subscribe pocketeer to push notifications service', function(done) {
      helpers.getAuthServer(wallet.pocketeers[0].id, function(server) {
        should.exist(server);
        server.pushNotificationsSubscribe({
          ...
          should.not.exist(err);
          server.storage.fetchPushNotificationSubs(wallet.pocketeers[0].id, function(err, subs) {
            should.not.exist(err);
            should.exist(subs);
    ...
    it('should allow multiple subscriptions for the same pocketeer', function(done) {
      helpers.getAuthServer(wallet.pocketeers[0].id, function(server) {
        should.exist(server);
        server.pushNotificationsSubscribe({
          token: 'DEVICE_TOKEN',
    ...
    it('should unsubscribe pocketeer to push notifications service', function(done) {
      helpers.getAuthServer(wallet.pocketeers[0].id, function(server) {
        should.exist(server);
        async.series([
  ...
  describe('Tx confirmation notifications', function() {
    ...
    it('should subscribe pocketeer to a tx confirmation', function(done) {
      helpers.getAuthServer(wallet.pocketeers[0].id, function(server) {
        should.exist(server);
        server.txConfirmationSubscribe({
          ...
          should.not.exist(err);
          server.storage.fetchActiveTxConfirmationSubs(wallet.pocketeers[0].id, function(err, subs) {
            should.not.exist(err);
            should.exist(subs);
            subs.length.should.equal(1);
            var s = subs[0];
    ...
    it('should unsubscribe pocketeer to the specified tx', function(done) {
      helpers.getAuthServer(wallet.pocketeers[0].id, function(server) {
        should.exist(server);
        async.series([
    ...
    beforeEach(function(done) {
      helpers.createAndJoinWallet(1, 1, function(s, w) {
        ...
        helpers.createAndJoinWallet(1, 1, {
          ...
          server.bch = s;
          wallet.bch = w;
          w.pocketeers[0].id.should.equal(TestData.pocketeers[0].id44bch);
          done();
        });
      });
    });
