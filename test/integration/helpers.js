...
helpers.before = function(cb) {
  function getDb(cb) {
    if (useMongoDb) {
      ...
      mongodb.MongoClient.connect('mongodb://localhost:27017/apws_test', function(err, db) {
        if (err) throw err;
        return cb(db);
      });
    } else {
...
helpers.getAuthServer = function(pocketeerId, cb) {
  ...
  WalletService.getInstanceWithAuth({
    ...
    verifyStub.restore();
    if (err || !server) throw new Error('Could not login as pocketeerId ' + pocketeerId + ' err: ' + err);
    return cb(server);
  });
};

helpers._generatePocketeersTestData = function() {
  ...
  _.each(xPrivKeys, function(xPrivKeyStr, c) {
    ...
    var id45 = Model.Pocketeer._xPubToPocketeerId('btc', xpub_45H.toString());
    ...
    var id44btc = Model.Pocketeer._xPubToPocketeerId('btc', xpub_44H_0H_0H.toString());
    var id44bch = Model.Pocketeer._xPubToPocketeerId('bch', xpub_44H_0H_0H.toString());
...
helpers.getSignedPocketeerOpts = function(opts) {
  var hash = WalletService._getPocketeerHash(opts.name, opts.xPubKey, opts.requestPubKey);
  opts.pocketeerSignature = helpers.signMessage(hash, TestData.keyPair.priv);
  return opts;
};
...
helpers.createAndJoinWallet = function(m, n, opts, cb) {
  ...
  server.createWallet(walletOpts, function(err, walletId) {
    ...
    async.each(_.range(n), function(i, cb) {
      var pocketeerData = TestData.pocketeers[i + offset];
      var pocketeerOpts = helpers.getSignedPocketeerOpts({
        walletId: walletId,
        coin: opts.coin,
        name: 'pocketeer ' + (i + 1),
        xPubKey: (_.isBoolean(opts.supportBIP44AndP2PKH) && !opts.supportBIP44AndP2PKH) ? pocketeerData.xPubKey_45H : pocketeerData.xPubKey_44H_0H_0H,
        requestPubKey: pocketeerData.pubKey_1H_0,
        customData: 'custom data ' + (i + 1),
      });
      if (_.isBoolean(opts.supportBIP44AndP2PKH))
        pocketeerOpts.supportBIP44AndP2PKH = opts.supportBIP44AndP2PKH;

      server.joinWallet(pocketeerOpts, function(err, result) {
        should.not.exist(err);
        pocketeerIds.push(result.pocketeerId);
        return cb(err);
      });
    }, function(err) {
      if (err) return new Error('Could not generate wallet');
      helpers.getAuthServer(pocketeerIds[0], function(s) {
